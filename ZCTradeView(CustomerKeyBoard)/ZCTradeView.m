...
- (void)setupInputView
{
    ...
    self.bgView = [[UIView alloc] initWithFrame:CGRectMake((ZCScreenWidth -302)/2, (ZCScreenHeight -180)/2, 302, 190)];
    self.bgView.backgroundColor = [UIColor whiteColor];
    self.bgView.layer.cornerRadius = 10;
    [self addSubview:_bgView];
    
    
    ZCTradeInputView *inputView = [[ZCTradeInputView alloc] initWithFrame:CGRectMake((ZCScreenWidth -302)/2, (ZCScreenHeight -180)/2, 302, 180) title:_titleStr payType:_payType money:_payMoney cardInfo:_cardInfo];
...
- (void)showKeyboard
{
    ...
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.bgView.transform = CGAffineTransformMakeTranslation(0, marginTop - self.bgView.y);
...
- (void)cancle
{
    [self hidenKeyboard:^(BOOL finished) {
        ...
        [UIAlertView showWithTitle:@"是否取消交易?" message:nil delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
    }];
}
